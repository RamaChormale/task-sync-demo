...
export default function TaskTable({ tasks, onEdit, onClose, onResolveConflict, resolving }) {
  ...
  return (
    <>
        ...
        <table className="w-full text-sm">
          ...
          <tbody className="divide-y divide-gray-100 bg-white">
            {tasks.map((task) => {
              ...
              return (
                <tr key={task._id} className={`hover:bg-gray-50 transition-colors ${task.syncStatus === 'conflict' ? 'bg-orange-50' : ''} ${isClosed ? 'opacity-70' : ''}`}>
                  <td className="px-4 py-3">
                    <div className={`font-medium max-w-[200px] truncate ${isClosed ? 'text-gray-400 line-through' : 'text-gray-800'}`}>{task.title}</div>
                  ...
                  <td className="px-4 py-3 text-gray-500 whitespace-nowrap">
                    {isClosed ? formatDate(task.closedAt) : formatDate(task.createdAt)}
                  </td>
                  ...
                  <td className="px-4 py-3">
                    <div className="flex items-center gap-1">
                      {!isClosed && <Button variant="ghost" size="sm" onClick={() => onEdit(task)}>Edit</Button>}
                      {!isClosed && (
                        <Button variant="ghost" size="sm" className="text-orange-600 hover:bg-orange-50" onClick={() => onClose(task)}>
                          Close
                        </Button>
                      )}
                      {isClosed && <span className="text-xs text-gray-400 px-2">Closed</span>}
      ...
      <GithubIssueModal task={issueTask} isOpen={!!issueTask} onClose={() => setIssueTask(null)} />
    </>
  );
}
